...
|---|---|---|---|
| **1** | Enthalpy from experimental data (mcΔT) + Hess's Law | 9/9 | 6-12 |
...
| **5** | Equilibrium (Kc expression and calculation) | 8/9 | 3-6 |
...
**Do not skip priorities 1-3.** Every single Paper 3 has tested enthalpy from data, mole calculations, and redox. Together with transition metal complexes and Kc, these five topics typically account for 25-40 marks per paper.
...
Do not present rows of unlabelled numbers. Write what each number represents. "n(NaOH) = 0.0232 ÷ 1000 × 23.20 = ..." is a calculation. "0.0232 × 23.20 = ..." is guesswork the examiner cannot follow. This is flagged in **every single examiner report** from 2017 to 2024.

### Rule 2: Use mass of solution, not mass of solid, in mcΔT
...
m³ to dm³ (×1000), cm³ to dm³ (÷1000), kJ to J (×1000), °C to K (+273). Every year, candidates lose marks by getting unit conversions wrong. Flagged 2017, 2018, 2019, 2020, 2021, 2022, 2024.
...
If you took a 25 cm³ aliquot from a 250 cm³ flask, multiply by 10. If from 100 cm³, multiply by 4. Forgetting to scale is one of the most common titration errors. Flagged 2020, 2021, 2022, 2023.

### Rule 8: Use concordant titres for the mean
Do not average all three titres. If two titres agree within 0.20 cm³ and the third is an outlier, use only the concordant pair. Flagged 2018, 2020.

### Rule 9: Learn common ion formulae
NH₄⁺ (not NH₃⁺), SO₄²⁻ (not SO₄⁻), NO₃⁻, CO₃²⁻, Na₂CO₃ (not NaCO₃). Wrong ion formulae cost marks every time an equation is written. Flagged 2021, 2023, 2024.
...
### Enthalpy Calculations (mcΔT + Hess's Law)
...
- **Sign of ΔH**: Exothermic reactions have a negative ΔH. Temperature rises → exothermic → negative. Do not omit the sign.
- **ΔH per mole**: After calculating q = mcΔT, divide by moles of the substance specified. For ΔH_neut, this is per mole of water formed (2018).
...
- **Square brackets mean concentration**: Kc uses [X] in mol dm⁻³. If you are given moles and volume, divide first.
...
- **Square roots**: If Kc = [A][B] and [A] = [B], then [A]² = Kc, so [A] = √Kc. Many candidates forget the square root (2021, 2025).
- **Only temperature changes Kc**: Pressure, concentration, and catalyst changes shift equilibrium position but do NOT change Kc.
- **Units of Kc**: Always derive units from the expression. Do not assume "no units."
...
- **Rate = −gradient** (for concentration of reactant): If the graph shows decreasing concentration over time, rate is the negative of the gradient.
...
- **Rate constant units**: Derive algebraically from rate = k[A]^m[B]^n. Do not memorise.
- **ln k vs 1/T graphs**: Gradient = −Ea/R. The y-intercept = ln A. Many candidates confuse gradient and intercept (2020).
...
- **Strong acid pH**: pH = −log[H⁺]. For HCl, [H⁺] = concentration. For H₂SO₄, [H⁺] = 2 × concentration.
- **Strong base pH**: pOH = −log[OH⁻], then pH = 14 − pOH. For Ba(OH)₂, [OH⁻] = 2 × concentration (2024, 2025).
- **Weak acid pH**: [H⁺] = √(Ka × [HA]). Take the square root. Give pH to 2 decimal places.
- **Buffer pH**: pH = pKa + log([A⁻]/[HA]). Do NOT use the weak acid method (square root) for buffers. Swapping [HA] and [A⁻] gives a wrong pH (2024).
- **Dilution**: When diluting, use c₁v₁ = c₂v₂. Do not divide by the volume added; divide by the total new volume (2024).
...
- **Include the charge**: [Cu(NH₃)₄(H₂O)₂]²⁺ must include the 2+ charge. Omitting the charge loses a mark.
...
- **Ligand substitution equations**: Must be balanced with correct charges. CuCl₄²⁻ is tetrahedral, not square planar (2020).
- **Optical isomerism in complexes**: Two optical isomers (non-superimposable mirror images) exist for tris-bidentate octahedral complexes like [Fe(C₂O₄)₃]³⁻. Draw both mirror images.

### Redox Chemistry

- **Half-equation balancing**: Add H₂O to balance O, then H⁺ to balance H, then e⁻ to balance charge.
...
- **Standard solution preparation**: Calculate mass → dissolve in **less** than the final volume of distilled water → transfer quantitatively to volumetric flask → wash beaker and glass rod into flask → make up to the mark → invert to mix. Flagged 2021, 2024.
...
- **Percentage uncertainty**: For a burette, uncertainty = ±0.05 cm³ per reading. A titre involves two readings, so total uncertainty = ±0.10 cm³. Many candidates forget the ×2 (2019).
...
- [ ] mcΔT uses **mass of solution**, not mass of solid
- [ ] All temperatures converted from **°C to K** (+273)
- [ ] All volumes converted correctly (cm³ ÷ 1000 = dm³)
- [ ] Enthalpy values have the **correct sign** (negative for exothermic)
- [ ] Kc expressions use **square brackets** for concentrations
...
- [ ] Titration mean uses **concordant titres** only (within 0.20 cm³)
...
- Kc calculation and equilibrium conditions for methanol synthesis
...
| **Weak-acid pH calculation** | Not on Paper 2. pH appears on Paper 3 in 7/9 years. Note: buffers specifically are *rare* on OCR Paper 3 (only once, 2024, 2 marks), so a weak-acid pH or strong base pH is the more likely angle than a full buffer question. |
...
- Equilibrium (Kc calculation or qualitative Le Chatelier)
- Bond angles / VSEPR
...
*HSJ Tuition / ChemMastery. Internal resource. Based on OCR H432/03 2017-2025 + 2026 Papers 1 & 2 intelligence.*
